...
public class ManagerLoginFilter implements Filter{
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
		...
		//매니저 홈 이동 필터
		if(session.getAttribute("loginManager")==null) { //session ID에 loginManager 있는지 확인 없다면
			if(response instanceof HttpServletResponse){ //response도 형변환 할 수 있는지 확인
				((HttpServletResponse)response).sendRedirect("/obo/m/"); //매니저 홈으로 이동한다.
			}
			return;
		}
		chain.doFilter(request, response);
		//요청후
		log.debug("===== manager 로그인 필터 작동!");
